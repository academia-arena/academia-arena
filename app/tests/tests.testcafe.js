...
test('Test that user can view the pull page after sign in', async (testController) => {
  // Sign in
  await navBar.gotoSignInPage(testController);
  await signinPage.signin(testController, credentials.username, credentials.password);
  await navBar.isLoggedIn(testController, credentials.username);

  // Navigate to pull for cards page
  await pullPage.isDisplayed(testController);

  // Wait for the page to fully load and complete any actions
  await testController.wait(5000); // wait for 5 seconds
